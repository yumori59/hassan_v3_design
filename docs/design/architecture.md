...
| **MR-2** | **Vercel の Root Directory + Ignored Build Step** | Root Directory = `frontend`。Ignored Build Step で **①`main` / `production` 以外のブランチはビルドしない** (2026-09-07。feature ブランチの検証は dev = PR 単位プレビュー — [infrastructure.md](infrastructure.md) INF-U) **かつ ②`frontend/` と `api/` に差分が無ければビルドしない** | backend だけの PR でも毎回 Preview ビルドが走る |
...
(テーブル 44 + 例外 11 / 設計判断 DM-1〜DM-20 / 採番と冪等性 / 台帳 / 派生物の無効化 /
...
| D-1 環境 | 部分 | local / dev / staging / prod の 4 環境 (ユーザー指定。2026-09-07 に 3 → 4 = [infrastructure.md](infrastructure.md) INF-U。dev は PR 単位プレビュー、staging が `main` の継続デプロイ先)。**環境間の変更の切り分け** (staging の未リリース変更を prod に出さない) は運用設計で確定。環境で変わる設定値の持ち方は §3.9② |
