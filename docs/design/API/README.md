...
> [auth-accounts.md](auth-accounts.md) は同じ規約に載るが、**認証系であるがゆえの差分 4 点**を持つ:
> ①**公開エンドポイント 8 本がある** (**2026-08-29 の AA-D-30 で 6 → 8**) (§2.1 の「本ディレクトリに公開エンドポイントは無い」の例外)
> ②**資格情報エラーの 401 に `CodedError` 本文を持つ** (§2.5 の「401 は本文なし」の例外)
> ③**429 を返す 14 本がある** (§2.5 の 429 行が「本ディレクトリの対象外」としている範囲の例外。
> 未認証で叩ける 8 本 + MFA 検証 1 本 + **認証済みで資格情報を提示する 5 本** = ユーザーのメール変更 / パスワード変更 / MFA 再登録 + **社内管理者のメール変更 / パスワード変更** (2026-08-29 の AA-D-33)。**2026-08-10 の AA-D-22 で `POST /admin/mfa/totp/verify` が消え MFA 検証は 1 本**。**2026-08-29 の AA-D-30 で公開が 6 → 8 本になり 10 → 12、同日の AA-D-33 で社内管理者の自己管理 2 本 (メール変更・パスワード変更) が加わり 12 → 14**)。
> ④**単数形の単一リソースパス 3 本がある** (`GET /contract` / `GET`・`PUT /company` / `PUT /company/mfa` —
> §1.2 D-API-2 の「複数形に統一」の例外。**ID 不要で認証コンテキストから一意に解決される契約内単一リソース**は
> 単数形で表す = 2026-08-15 の **AA-D-25**。パスパラメータを持たないため §1.4 の `{id}` 検査には影響しない)。
> 加えて **403 の第 3 系統 (契約の不変条件ガード)** を持つため、§2.2 / §2.5 の「403 は R-1 / R-2 の
> 2 系統・合計 17 本」は**9 ドメインについての数**である。差分の根拠は
...
| D-API-2 | パス命名 | **kebab-case + 複数形**に統一。パスパラメータは常に `{<単数リソース名>_id}` (`{theme_id}` `{asset_id}` `{board_id}`)。ネストは 2 段まで。**例外**: ID 不要の契約内単一リソース (`/contract` / `/company`) は**単数形** ([auth-accounts.md](auth-accounts.md) AA-D-25。§0 の差分④) | (a) v2 の現状踏襲 (F-2): `/company-mission` `/event_logs` `:id`/`:hassan_id` の不統一がそのまま orval 生成関数名の揺れになり、実装者が毎回既存例を探す。(b) `{id}` で統一: ネスト時 (`/idea-boards/{id}/items/{id}`) に衝突し、gin がルートを登録できない |
...
| # | 系統 | 判定の根拠 | 該当 | v2 の前例 |
|---|---|---|---|---|
| R-1 | **契約内の管理者/メンバー区別** (`accounts.auth_role_id`) | `authAccount.AuthRoleID.IsAdmin()` | [settings.md](settings.md) §3.1 の **4 本** (`PUT /settings/workspace` / `GET /usage-summary` / `GET /activity-logs` / `PUT /settings/eval-criteria` — **2026-08-24 追加。PV-D5**) | `hassan-v2-backend/controller/event_logs.go:47-50` が同じ判定で `forbidden(ctx, apperror.RequestUserNotAdmin())` を返す。ロール定義は `hassan-v2-backend/entity/auth_role.go:7-10` (1 = 管理者 / 2 = メンバー) |
...
**「使う。ただし用途は R-1 の 4 本に限る」**を提案する (詳細は [settings.md](settings.md) §3.1。2026-08-24 に 3 → 4 — PV-D5)。
...
**合計 17 本** = [settings.md](settings.md) の 4 本 (R-1。**2026-08-24 に 3 → 4** — PV-D5) + [idea-boards.md](idea-boards.md) の 8 本 + [ideas.md](ideas.md) の 5 本 (R-2、計 13 本)。
...
同書の 403 は 16 本ある** — 系統と本数の SSOT は同書 §3.1 (**実測は `make check-endpoint-mapping` が正**。2026-08-10 の AA-D-22 で 10 → 8、**2026-08-25 の AA-D-26 で 8 → 12、2026-08-26 の AA-D-27 で 12 → 13、2026-08-29 の AA-D-29 で 13 → 15、同日の AA-D-30 で 15 → 16**)。
**第 4 系統 (R-4 = 社内管理者の SuperAdmin 限定) も同書だけが持つ** — §2.4 の②契約管理 5 本と③管理者アカウント管理 3 本が該当する。
...
| 状況 | コード | 本文 | 適用範囲 |
|---|---|---|---|
| `X-Token` 欠落・不正・期限切れ / アカウント不存在・ロック・**無効化** ([auth-accounts.md](auth-accounts.md) の AA-D-32) / MFA 未検証 | **401** | なし | **9 ドメイン (themes / assets / knowledge / idea-boards / news / settings / conversation / ideas / plans) の全エンドポイント**。**[auth-accounts.md](auth-accounts.md) は例外** — **トークンの発行・昇格を目的とする経路** (サインイン 2 本 / MFA 検証 2 本) の資格情報不一致を 401 + `CodedError` 本文で返す。**FE の「401 → 強制サインアウト」の分岐は本文のコード接頭辞で切り分ける** (`AU-T-` = 破棄 / `AU-C-` = フォーム内)。規則と値域は同書 §3.1 / §3.1.1 が持つ。**認証済みの状態変更に添える本人確認の不一致 (現在パスワード・MFA 再登録のコード) は 401 ではなく 400** — 下の 400 行に含まれる (同書 AA-D-17) |
...
| **契約内管理者限定の操作を一般メンバーが叩いた** (R-1) | **403** | `CodedError` | [settings.md](settings.md) §3.1 の **4 本** |
...
| **レート制限の超過** | **429** | `CodedError` (`Retry-After` 付き) | **9 ドメインは対象外** (全て認証必須 = §2.1。認証済み経路の暴走抑止は O-3 の安全弁が担う — [../observability.md](../observability.md) §4.4)。**[auth-accounts.md](auth-accounts.md) は 429 を返す (14 本。実測は `make check-endpoint-mapping` が正)** — 同書 §3.7 が対象エンドポイントを列挙する (未認証で叩ける 8 本 + MFA 検証 1 本 + 認証済みで資格情報を提示する 5 本 = ユーザー側 3 本 + 社内管理者の自己管理 2 本)。値と方式の SSOT は [../auth.md](../auth.md) §6.11-3。**403 で返さない** |
...
**合計 160 エンドポイント** = 下表の 9 ドメイン **114 本** (**2026-08-24 に settings +2 — PV-D5**) + 認証・アカウント基盤
[auth-accounts.md](auth-accounts.md) の **46 本** (同書 §2。**2026-08-10 の AA-D-22 / AA-D-13 で 37 → 33**、
**2026-08-25 の AA-D-26 (契約管理 4 本の追加) で 33 → 37、2026-08-26 の AA-D-27 (招待の再発行) で 37 → 38、
2026-08-29 の AA-D-29 (社内管理者アカウントの作成・権限変更) で 38 → 40、
同日の AA-D-30 (パスワード登録の公開 2 本 + 登録リンクの再発行 1 本) で 40 → 43、
**同日の AA-D-33 (社内管理者の自己管理 3 本) で 43 → 46**)。

> **§1・§2 の共通規約が対象にするのは 9 ドメインの 114 本**である (§0 の注)。
> 認証・アカウント基盤は D-ST-1' (2026-07-30) により v3 が実装し、
> **入出力仕様は [auth-accounts.md](auth-accounts.md) で確定した** (2026-07-31。plan.md の **Task-3i**)。
> 工数・依存の見積りでは**同書の本数を必ず加算すること** (**値は上の総覧表と同書 §2 の見出しが正** —
> DR-9。2026-08-29 に「37 本」が実数とずれていたのを是正し、数値を落として参照に置き換えた)。
...
| 設定 | [settings.md](settings.md) | **8** | 0 | 0 | **4** |
...
| 認証・アカウント基盤 | [auth-accounts.md](auth-accounts.md) | **46** | 0 | 0 | **16** |
| **合計** | — | **160** | **9** | **6** | **33** |
...
**認証・アカウント基盤の 16 本は別勘定** = 契約内管理者限定 8 本 (同書 §2.3) + **SuperAdmin 限定 8 本 (同 §2.4 の②契約管理 5 本 = AA-D-26 / AA-D-27 と、③他の管理者アカウントの管理 3 本 = AA-D-29 / AA-D-30)**。**同 §2.4 の④自己管理 3 本 (2026-08-29 の AA-D-33) は `admin` ロールでも実行できるため 403 を返さず、本数は 16 のまま**。**旧 SuperAdmin 限定の 1 本 (`POST /admin/admins/{id}/mfa/reset`) は AA-D-22 で消滅**。
...
| GET | `/settings/workspace` | v3 側ワークスペース設定取得 (アセット可視性の既定 — ST-Q8) | 契約 | **1** (**2026-08-24 是正** — ST-Q8=a は 2026-08-12 に撤回済み。[../auth.md](../auth.md) §6.12 (c) / [settings.md](settings.md) §3 と揃えた) |
| PUT | `/settings/workspace` | v3 側ワークスペース設定更新 (**契約内管理者のみ / 403**) | 契約 | **1** (同上) |
| GET | `/usage-summary` | 契約の利用量集計 (月 × メンバー × 活動種別のクロス集計 — ST-Q9。**契約内管理者のみ / 403**) | 契約 | 1 |
| GET | `/activity-logs` | 契約の活動ログ一覧 (**契約内管理者のみ / 403**) | 契約 | 1 |
| GET | `/settings/eval-criteria` | 契約の評価基準取得 (**2026-08-24 追加 — PV-D5**。行なしは 200 + 既定基準) | 契約 | 1 |
| PUT | `/settings/eval-criteria` | 同 更新 (**契約内管理者のみ / 403**) | 契約 | 1 |
...
| A-2 ロールと適用範囲 | **回答** | §2.2。認証ロールは `AuthRoleUser` のみ。その上で **R-1 契約内管理者限定 4 本** ([settings.md](settings.md) §3.1) と **R-2 リソース単位ロール / 所有者 13 本** ([idea-boards.md](idea-boards.md) §3.1 の 8 本 + [ideas.md](ideas.md) §1.3 の 5 本) が 403 を返す |
...
| A-5 ステータスコード | **回答** | §2.5 (= **AC-1.4**)。判定基準は「**見えるリソースへの権限不足 = 403 / 見えないリソース = 404**」。403 は合計 **17 本** (R-1 4 + R-2 13。**2026-08-24 に `PUT /settings/eval-criteria` を追加** — PV-D5)。SSE 開始後の失敗は HTTP で表現しない (D-API-12) |
