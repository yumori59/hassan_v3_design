...
| GET | `/ideas/{idea_id}/evaluation` | **リッチ評価の取得** (stale 判定つき) | 取得と同じ | R: `{evaluation, stale, status, source_idea_version_id, evaluated_at, criteria_version, failure?}` (§2.5。`criteria_version` は **2026-08-24 PV-D5 で追加** — §6.3.4) | 200 / 404 | — |
...
| **評価基準の契約ごとの編集 / 変更履歴** | **無い** | **無い** | **`_evalCriteriaState` に `history` あり**。コメントに「組織で共有・管理者のみ編集可」(F-PV7) | ~~増分 2 へ先送り~~ → **編集は増分 1 へ前倒し (2026-08-24 PV-D5)・履歴のみ増分 2** | 前倒しの決定と読む側の規則は §6.8。履歴を送った理由も同節の② |
...
**2026-08-24 改訂 (PV-D5 / AC-PV-1.2 の改訂)**: 契約単位の基準を `eval_criteria_settings` に保存できるようになったため、
**Go 定数は「契約行が無い場合の既定値の SSOT」へ縮小**した。契約行があればそれが採点に使われる (§6.8 の読み方)。

| 置き場所 | 何を持つか |
|---|---|
| **`entity/idea` の定数** (**既定値**の SSOT) | 軸の重み (40 / 35 / 25) / サブ基準の配点 (4 / 3 / 3) / **数値サブ基準の閾値表** / アンカーの点数集合 / 判定バンド (§6.3.3)。**契約行が無いときの採点と、行なし `GET /settings/eval-criteria` の応答**に使われる |
...
| **DB** (`eval_criteria_settings`。**2026-08-24 前倒し — PV-D5**) | **契約が編集した基準** (jsonb。形と検証は [settings.md](settings.md) §3 / D-ST-8)。~~持たない~~ (旧記述。増分 2 まで持たない判断だった) |
| **`settings` API** | **持たない** |
...
| # | 却下案 | 理由 |
|---|---|---|
| (a) | **契約ごとに DB (`evaluation_criteria` テーブル等) で持つ** | v4 のコメント (`同:17060` 「組織で共有・管理者のみ編集可」) が示す最終形だが、**DB スキーマ + settings API + 変更履歴テーブルを伴い増分 1 の枠に入らない** (PV-D1 の却下 C)。~~増分 2 へ先送りする~~ **2026-08-24 改訂 (PV-D5)**: 履歴を除く最小形 (`eval_criteria_settings`) を前倒しで採用した — Go 定数は既定値の SSOT として残る (§6.8) |
| (b) | **`settings` に持つ (契約単位の設定値として)** | 同上に加え、**評価のたびに設定を読む経路**が増え、`entity` の純粋関数が I/O に依存する (L-1 の依存規則違反)。**2026-08-24 注 (PV-D5)**: 前倒し後もこの却下理由は「重み・配点・閾値を引数で受け取る純粋関数」(§6.8 の①) で回避されている — DB を読むのは usecase/service 層で、`entity` は I/O を持たない |
...
| **`criteria_version`** | **Go** (`entity/idea` の定数、**契約行があれば `eval_criteria_settings.criteria_version`** — 2026-08-24 PV-D5) | どの軸定義・どの重みで採点したかの記録 (§6.8 の②)。**jsonb には入れず `idea_evaluations` の列に持つ**。~~増分 1 では API で返さない — 値が 1 種しかなく~~ **2026-08-24 再確定 (PV-D5)**: 契約が基準を編集できるようになり「値が 1 種」の前提が消えたため、**旧記述の再検討条件どおり `GET /ideas/{idea_id}/evaluation` の応答に `criteria_version` を足す** (詳細応答のみ。一覧 §2.1 には足さない — 軽い値の趣旨)。FE は「現行基準と違う版で採点された」表示 (stale 判定の材料) に使える |
...
### 6.8 増分 2 への先送り (AC-PV-7.1 / AC-PV-7.2 — **①③は 2026-08-24 に前倒し。PV-D5 / AC-PV-8.x**)

**本節が回答する AC: AC-PV-7.1 / AC-PV-7.2** (+ 前倒し後の読む側の規則 = AC-PV-8.4)。**無言で対象外にしない** (DR-2)。

**先送りする項目 (名指し)** — **①③は 2026-08-24 のユーザー決定 (PV-D5) で最小形を増分 1 へ前倒しした**。
残る先送りは**②のみ**:

| # | 項目 | 増分 1 で対象外にする理由 | 先送り先 |
|---|---|---|---|
| ① | **評価基準を契約ごとに編集する機能** (v4 の `_evalCriteriaState` の永続化 — 軸・重み・サブ基準の配点・アンカーを契約単位で書き換える) | ~~増分 2~~ → **増分 1 へ前倒し (2026-08-24 PV-D5)**。旧理由のうち「管理者のみ編集可 = v3 に無いロール粒度」は実装リポ #52 の `IsAdmin()` (403) で解消。保存先は `eval_criteria_settings` ([../data-model.md](../data-model.md) §4.9) | **増分 1** (実装リポ #110) |
| ② | **評価基準の変更履歴** (v4 の `history` — 誰がいつ何を変えたか。`docs/prototype/hassan_agent_prototype_v4.html:17120`〜`:17125`) | ①の派生。**履歴テーブルと「過去の基準で採点された評価」の関係**を決める必要があり、前倒しには含めない (PV-D5 の却下 (b)) | **増分 2** |
| ③ | **`settings` の評価基準 CRUD** (画面 + API) | ~~増分 2~~ → **増分 1 へ前倒し (2026-08-24 PV-D5)**。`GET/PUT /settings/eval-criteria` として [settings.md](settings.md) §3 / §3.1 / D-ST-8 で確定 (エンドポイント +2 本と `make check-endpoint-mapping` の期待値更新は同じ差分で実施 — PV-DF2) | **増分 1** ([settings.md](settings.md) が SSOT) |

**前倒し後の読む側の規則 (AC-PV-8.4 / BE-10)**: 評価 (C-6) は採点のたびに
①契約行 (`eval_criteria_settings.criteria`) があればそれを ②無ければ `entity/idea` の Go 定数の既定を、
**引数で受け取る純粋関数**に渡して使う (下表①の準備がそのまま効く。キャッシュは持たない — 下表③)。
**書く側 (`PUT /settings/eval-criteria`) 単独のマージでは採点に反映されない** — 読む側の接続は C-6 の issue が担う。

**実施時期は本書で決めない** — [schedule-2026q3.md](../../../aidlc-docs/schedule-2026q3.md) が唯一の SSOT
(PV-Q8 の判断ルール③。時期を 2 箇所に書かない)。

**増分 2 を見越した増分 1 の制約 (AC-PV-7.2)** — **2026-08-24 の前倒し (PV-D5) により、
下表の「増分 2 で何が変わるか」のうち①の SSOT 移動・**②の版の意味の拡張 (「契約 ID + 基準の版」の発生)**・③の DB 読みは増分 1 内で起きる**ことになった。
**制約そのもの (引数渡しの純粋関数 / `criteria_version` / キャッシュなし) は変わらず有効**で、
前倒し実装の受入条件になる (AC-PV-8.3 / AC-PV-8.4):
...
| ② | **どの基準で採点されたかの記録** | **`idea_evaluations.criteria_version`** (text・NOT NULL) を**増分 1 から持つ**。値は契約行 (`eval_criteria_settings.criteria_version`) があればその値、無ければ `entity/idea` の定数の識別子 (~~増分 1 は 1 種のみ~~ **2026-08-24 PV-D5 で「契約 ID + 版」が増分 1 から発生する** — [settings.md](settings.md) D-ST-8) | **「契約 ID + 基準の版」を指す形への意味の拡張は、前倒しにより増分 1 で起きた** (2026-08-24 PV-D5。旧記述はこれを増分 2 の出来事としていた。列は追加しない — 増分 2 で残るのは変更履歴のみ)。**却下 (a) 何も持たない**: 増分 2 で重みを変えた瞬間に、既存の `composite` がどの重みで計算されたか分からなくなり、**過去のスコアと比較できず** IDA-R7 の A/B も成立しない。**却下 (b) 重みの値そのものを jsonb で保存する**: 増分 1 では常に同一値の冗長データで、同一性の判定が値の比較になる |
...
| **D-IDA-23** | **軸定義・重みの SSOT と増分 2 への準備** (AC-PV-1.2 / AC-PV-7.2。**2026-08-24 改訂 — PV-D5**) | **`entity/idea` の Go 定数を既定値の SSOT とし、計算関数は重み・配点・閾値を引数で受け取る純粋関数にする**。採点に使った基準は **`idea_evaluations.criteria_version`** に記録する (§6.8)。**2026-08-24 改訂**: 契約が編集した基準は `eval_criteria_settings` が持ち、契約行 > Go 定数 (既定) の順で採点に使う (§6.3.1 / §6.8) | (a) **DB / `settings` に持つ**: ~~増分 1 の枠に入らない (§6.8 の①)~~ **2026-08-24 に前倒しで採用** (PV-D5。却下理由は解消 — §6.8 の①)。(b) **定数を関数の中で直接参照する**: 増分 2 で `entity` が DB を読む形になり L-1 の依存規則に反する。(c) **`criteria_version` を持たない**: 増分 2 で重みを変えた瞬間に、既存の `composite` がどの重みで計算されたか分からなくなる。(d) **重みの値そのものを jsonb で保存する**: 増分 1 では常に同一値の冗長データで、同一性の判定が値の比較になる |
