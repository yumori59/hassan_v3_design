...
**403 の 12 本は契約内管理者限定 (R-1) 8 本と SuperAdmin 限定 (R-4) 4 本**
(**実測は `make check-endpoint-mapping` が正**。2026-08-10 の AA-D-22 / AA-D-13 で 9 → 8、
**2026-08-25 の AA-D-26 で §2.4 の契約管理 4 本が加わり 8 → 12**)。
R-1 の 8 本のうち 3 本は**契約の不変条件ガード (R-3。§3.4)** でも 403 を返す。
**R-4 = 社内管理者の SuperAdmin 限定** — §2.4 の②契約管理が該当し、`admin` ロールでは 403 になる。
...
| PUT | `/admin/contracts/{contract_id}` | 契約の更新 (会社名・人数・期間・代表者・部署) | B: 作成と同じ項目 — R: `AdminContractView` | 200 / **403** (SuperAdmin 以外) / 400 / **404** | `同:221`、`hassan-v2-backend/controller/company.go:318` |
...
| **A-2** ロールと適用範囲 | **回答** | ①**一般ユーザー** (`AuthRoleUser`) = §2.2 / §2.3 ②**契約内管理者** (R-1) = §2.3 の 8 本が 403 ③**社内管理者** (`X-Admin-Token`) = §2.4 の 9 本 ④**SuperAdmin** (R-4) = §2.4 の②契約管理 4 本 (**2026-08-25 の AA-D-26**。`POST /admin/admins/{id}/mfa/reset` は AA-D-22 で消滅したため、SuperAdmin 限定の対象はこの 4 本に入れ替わった)。**v2 で管理者限定だった操作 (V2-F12) はすべて維持している**が、**契約管理だけは v2 の実挙動 (Admin でも実行可) より狭めた** — v2 自身の宣言 (`hassan-v2-backend/entity/admin_auth_role.go:7-10` の「Admin: Read 機能を利用可能」) に合わせた結果であり、**C-16 に対する意図的な後退**として AA-D-26 の却下 (d) に記録した ([../auth.md](../auth.md) §9.3 Q-A2) |
