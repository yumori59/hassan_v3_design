...
| ~~**LM-R6**~~ | **調査完了・クローズ (2026-08-02)**。対照表の結論は [API/ideas.md](API/ideas.md) §6.2 が SSOT。**採った統合方針は 4 点**: ①**v2 のみに存在する軸は無い** (4 軸とも PoC 側に同等物がある) ため PoC の軸を土台にする ②**新規性とミッション整合は v2 の 4 段基準表を v3 のプロンプトに取り込む** (PoC は基準表が無くスコアがブレる) ③**市場規模・CAGR は値・出典・根拠を PoC の構造で持ち、スコアは Go の閾値表で計算する** (LLM に数値の桁を判定させない) ④**落とすのは 4 件** — 総合スコアの 0〜40 尺度 (0.0〜10.0 の重み付け平均へ) / OGP の `image` / モデル選択 / 言語切替 (全件に理由あり) | **§6.2 の 4 (評価プロンプトの 1 本化) の前提が確定した**。実装時期は P-5 の実装時 (後から V-2 を足す形にしない) |
