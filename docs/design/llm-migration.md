...
| 会話型アイデア創出 (発散・custom tool・企画書) | **API 設計は完了 (2026-08-02)** — [API/conversation.md](API/conversation.md) / [API/ideas.md](API/ideas.md) / [API/plans.md](API/plans.md) の 3 ファイル。LM-Q1 (P-3 の統合) と LM-Q2 (v2 の V-1〜V-3 / V-6 / V-12 の統合) の設計もこの 3 ファイルが担った | [API/README.md](API/README.md) §0 / §3 |
| `GET /companies/genai` (URL から企業情報) | LLM 経路として認識済み。移行先の判定は**本書**が担う | [API/settings.md](API/settings.md) §5 |

**計測対象の索引との関係**: [API/README.md](API/README.md) §3 の総覧が **API ディレクトリ内の LLM 経路の索引**であり、
**本数はそこが正** (2026-08-02 に会話・アイデア・企画書の 3 ドメインが加わって増えた。
**本書に本数を転記しない** = DR-9。`make check-endpoint-mapping` が総覧と各ファイルの実測を照合する)。
本書 §4 の表は**移植元 (PoC / v2) から見た全経路の索引**であり、API 側の索引と数え方が違う
(廃止する経路・統合して独立移植しない経路を含むため)。**どちらかが他方を上書きするものではない**。
...
**[templates/app-monorepo/backend/.claude/rules/05-architecture-coding-rules.md](../../templates/app-monorepo/backend/.claude/rules/05-architecture-coding-rules.md) の「LLM の使い分け」表と
...
>
> **2026-08-01 の改訂 (CV-Q1=B。企画書系 V-7〜V-11 の優先度を 2〜3 → 1 へ)**:
> 会話型アイデア創出の Inception でユーザーが **CV-Q1=B (v2 の企画書 18 エンドポイントすべてを第 1 リリース)** を選択した
> ([../../aidlc-docs/inception/productionization/questions-conversation.md](../../aidlc-docs/inception/productionization/questions-conversation.md) の CV-Q1 /
> 同 [requirements-conversation.md](../../aidlc-docs/inception/productionization/requirements-conversation.md) の R-CV-1)。
> 本書は V-8 / V-10 / V-11 を優先度 2〜3、V-9 を 3 と判定していたが、**この判定は C-16 (v2 の操作を落とさない・後ろ倒しもしない) の適用結果と衝突する**。
> **移行計画を読む実装リポが企画書系を第 1 リリースから外さないよう、優先度列を 1 に統一した** (V-7 も同じ企画書ドメインのため揃える)。
> **§7.1 の M-8 (v2 企画書系の移送) も RL-4 → RL-3 に前倒しになる** (同節の注を参照)。
>
...
| P-2 | アイデア発散 (**v3 はレンズ 2 階層に改訂** — [API/conversation.md](API/conversation.md) §4.1.1。**PoC は `output_mode` の `domain` / `trend` / `usage` / `spec` の 4 値 enum** だった。**2026-08-23 増分 proto-v4 で改訂。R-CVA-16 対応**) | Managed Agent (`DIVERGE_AGENT_ID`) + raw 経路。`claude_managed_agents/prompts/idea_diverge_system.md` | **Managed Agent** (**再発行対象**) | Q1=Yes (発散中に検索・アセット参照を行う)。**未配線の自前ツールループ版は廃止** (§4.3 の X-5) | **C-1** | 1 |
...
| V-7 | 企画書 ブラッシュアップ stage-1 (クエリ補強) | 同上 (§2-8) | **P-4 (Agent 版タブ生成) へ統合** (2026-08-02 改訂。[API/plans.md](API/plans.md) §6.3 が判定 = §9.2 の R-PL-7)。企画書の生成経路を 2 系統にしない | Q2=No (段が Go に固定) | **C-3** | **1** (2026-08-01 に 2〜3 から変更。CV-Q1=B) |
| V-8 | 企画書チャット | `hassan-v2-backend/usecase/business_plan/business_plan_chat.go` (§2-9。SSE) | **直接 API** | Q3=No (§3 の ※3。ツールを持たない chat) | **C-4** | **1** (2026-08-01 に 2〜3 から変更。CV-Q1=B) |
| V-9 | 企画書サムネイル生成 (画像) | `hassan-v2-backend/usecase/business_plan/generate_business_plan_thumbnail.go` (§2-10。プロバイダ直指定) | **直接 API (Gemini)**。**機能は維持する** (2026-07-31 のユーザー決定 = §9.1 の LM-Q3。Gemini が LM-D の例外として残る) | Q1〜Q3=No。LM-D の例外 (Anthropic に画像生成が無い) | **C-6** | **1** (2026-08-01 に 2〜3 から変更。CV-Q1=B) |
| V-10 | 企画書詳細 セクション分析 7 種 (競合 / PESTEL / 市場 / 仮説検証 / 法規制 / 評価サマリ / ブラッシュアップ) | `hassan-v2-backend/usecase/business_plan/detailed/web_research.go` ほか (§2-11) | **P-4 へ統合** (2026-08-02 改訂。同上)。**v2 の 7 セクションのうち 6 つは v3 の 8 タブへ 1:1 で写像でき、`technology_analysis` は v2 に生成経路が無い** (`hassan-v2-backend/usecase/business_plan/detailed/converter.go:64` が「TechnologyAnalysisは未実装のため除外」と明記) | Q2=No (セクションの集合と順序は Go が固定) | **C-2** | **1** (2026-08-01 に 2〜3 から変更。CV-Q1=B) |
| V-11 | 企画書詳細 Web リサーチ | 同 (§2-12。plan / draft / search / revision / critic の 5 フェーズ) | **P-4 へ統合 + `gateway/exa`** (2026-08-02 改訂。同上)。5 フェーズは P-4 のタブ生成の内部手順として持つ | Q2=No (§3 の ※1) | **C-3** + **C-5** | **1** (2026-08-01 に 2〜3 から変更。CV-Q1=B) |
...
**2026-08-01 に確定**: 会話型 API 設計 ([API/conversation.md](API/conversation.md) §7 の D-CV-13) が
**「吸収しない」と判定**したため、**§7.1 の M-7 は消滅せず、V-4 / V-5 の独立移送を維持する**
(§9.2 の LM-R8 は判定済みとしてクローズ)。
...
prompts/
  ...
  idea/           diverge.md                    ← レンズ別セクションを 1 ファイルで持つ (§6.2 の 1。
                                                    2026-08-23 増分 proto-v4 でレンズ 2 階層に改訂 —
                                                    API/conversation.md §4.1.1)。V-1 / V-3 を統合 (LM-Q2)
...
|---|---|---|---|
| 1 | **発散 4 軸が 2 系統に分裂** (生きている `idea_diverge_system.md` の `output_mode` 節 + 未配線の `prompts/diverge/patterns/*`)。文言が異なり同一挙動ではない | **生きている系統を正とし、未配線側は廃止** (§4.3 の X-5)。`prompts/idea/diverge.md` 1 本に**レンズ別セクション**を持つ形を維持する。**2026-08-23 増分 proto-v4 でレンズ構造を 2 階層に改訂**し、`usage` / `spec` の 2 節を落とし `customer_pain` / `megatrend` / `adjacency` / `policy_tailwind` の 4 節を足す — 詳細は本書で再定義せず [API/conversation.md](API/conversation.md) §4.1.1 (レンズ 2 階層) / §4.1.2 (帰結 5。R-CVA-16) を参照する | **文言の差分を移植前に確認する** — 未配線側にしか無い有用な指示が含まれる可能性がある (§9 の LM-R2) |
...
| 4 | **アイデア評価が 3 実装並存** (PoC の現行 + 旧 `evaluate.go` の 2 プロンプト) + **v2 側にも評価がある** (V-2) | 旧 2 実装を廃止 (X-2) し、**PoC 現行 (P-5) と v2 (V-2) を `prompts/idea/evaluate.md` 1 本に統合する** (**LM-Q2 = 統合するで確定**。V-2 は独立機能として移植しない)。**統合の実施時期は P-5 の実装時** — **統合後の評価軸は増分 proto-v4 で 3 軸に確定した** ([API/ideas.md](API/ideas.md) §6.2 / §6.3。2026-08-23 に §9 の LM-R6 が再クローズ)。**最初から 3 軸で 1 本を作り**、後から V-2 を足す形にしない | **解消 (2026-08-23)**。評価軸・出力スキーマの対照は [API/ideas.md](API/ideas.md) §6.2 (対照表) / §6.2.1 (旧 9 軸の行き先) が SSOT。**採否・重み・サブ基準は本書に転記しない** (DR-9) |
| 5 | **企画書生成の system prompt が 3 方式並存** (8 タブ一括 / per-tab の Go 組み立て / Agent 版 1 タブ)。MaxTokens も個別 | **Agent 版 (P-4) に統一**し、8 タブすべてを同じ経路に載せる。8 タブ一括は廃止 (X-4)。per-tab の Go 組み立ては `prompts/plan/tab.md` + タブ別引数に置き換える。**v2 の簡易モード (V-6) もここに統合する** (LM-Q2 = 統合するで確定。独立した streaming 経路を作らない) | **2026-08-02 に判定完了** ([API/plans.md](API/plans.md) §6.2 / §6.3 = §9.2 の R-PL-7): ①**簡易モード (V-6) は 8 タブへ写像できる** — v2 の 12 セクションは `service` + `bmc` の 2 タブに収まり、粒度差は再生成の `sections` パラメータで吸収する (v2 が持っていた「手動編集セクションの保護」を失わせないこと) ②**「V-10 は独立移送」という旧記述は撤回した** — V-7 / V-10 / V-11 も P-4 に統合する。**企画書の生成経路を P-4 と直接 API の 2 系統にしないため** |
...
> **⚠️ 未解決 (2026-08-01 に発生。LM-R10 として §9.2 に起票)**: **CV-Q1=B と本表の段割りが整合していない**。
> ユーザー決定により**企画書ドメイン (V-7〜V-11) の優先度は 1 = 第 1 リリース**になった (§4.2 の改訂) が、
> **本表は M-8 (v2 企画書系) を RL-4 = 併用期間に置いたまま**である。
> **「優先度」と「移送の段」は別物**であり、機械的に一致させてはいけない —
> **優先度 1 = v3 側にその機能を作る** / **RL-4 = v2 側のトラフィックをドメイン単位で v3 へ移す**。
> したがって「v3 に企画書機能がある状態で第 1 リリースし、v2 からの移送は RL-4 で行う」は**論理的には成立する**。
> **ただし成立するかどうかは v2 の既存企画書データの扱い次第**であり、本書では判定できない:
> ①第 1 リリース時点で v3 の企画書に**既存データが無い**なら、利用者は「企画書が空の v3」を見ることになり、
> C-16 の「操作を落とさない」は満たしても実質的な後退になる
> ②M-8 は **M-7 (V-4 / V-5) を前提**にしており、M-8 を前倒すなら M-7 も前倒す必要がある
> ③[operations.md](operations.md) §6.1 の RL 定義と §6.2 の移送タイミング (RL-3 の最初と RL-4 の各ドメイン切替時) に触る
>
> **判定の主体**: 会話型 API 設計の企画書ドメイン (`docs/design/API/plans.md`) と
> [operations.md](operations.md) の移行計画。**本書は優先度列のみを改訂し、段割りは変更していない** (推測で畳まない)。
...
| ~~**LM-R6**~~ | **再オープン (2026-08-23) → 3 軸で再クローズ** (増分 proto-v4 / AC-PV-6.6)。**2026-08-02 にクローズした結論** (~~v2 のみに存在する軸は無いため PoC の軸を土台にする / 新規性とミッション整合は v2 の 4 段基準表を取り込む / composite は 5 軸の重み付け平均~~) **は、ユーザーが v4 プロトタイプの 3 軸 (`market_appeal` / `advantage` / `feasibility`) を採用したこと (PV-D1〜PV-D3) により失効した**。**新しい結論 (対照表・旧 9 軸の行き先・統合後の出力契約) は [API/ideas.md](API/ideas.md) §6.2 / §6.2.1 / §6.3 が SSOT** — 本書では再定義しない | **§6.2 の 4 (評価プロンプトの 1 本化) の前提は 3 軸のまま成立する** (同節を本改訂で更新済み)。実装時期は変更なし (P-5 の実装時。後から V-2 を足す形にしない) |
...
| ~~**LM-R8**~~ | **判定済み (2026-08-01) — 吸収しない**。会話型 API 設計 ([API/conversation.md](API/conversation.md) §7 の **D-CV-13**) が「**V-4 / V-5 を `research_market` に吸収しない**」と判定した。**理由**: V-4 / V-5 は「**アイデア 1 件**に対する Web 検索で市場規模・CAGR を埋める」経路 (入力 = 確定したアイデア / 出力 = 特定フィールド) だが、`research_market` (P-8) は「**テーマに対する候補領域の抽出**」(入力 = クエリ / 出力 = 領域行) であり、**目的・入出力・呼び出し契機のすべてが違う**。吸収すると 1 つの tool の出力スキーマに 2 用途が混ざる (BE-12 の温床) | **§7.1 の M-7 は消滅しない** (V-4 / V-5 の独立移送は維持)。§4.2 の V-4 / V-5 行も変更なし |
...
| **LM-R10** | **CV-Q1=B により企画書系の優先度を 1 (第 1 リリース) に改訂したが、§7.1 の M-8 (v2 企画書系の移送) は RL-4 のままで整合していない** (2026-08-01 起票)。「優先度 = v3 に機能を作る段」と「M-x = v2 からトラフィックとデータを移す段」は別物であり機械的に一致させられないが、**両立するかは v2 の既存企画書データの扱いに依存する** (§7.1 の注に 3 論点を記載) | **判定の主体は `docs/design/API/plans.md` (企画書ドメインの設計) と [operations.md](operations.md) の移行計画**。本書は優先度列のみ改訂し、段割りは変更していない (推測で畳まない) |
