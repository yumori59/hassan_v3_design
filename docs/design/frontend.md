...
- **FE-Q7: 社内管理者経路の WAF による IP 制限をどうするか (新規 2026-07-30)**。
  ...
  **暫定既定**: ③(IP 制限を管理者経路について諦める)。⚠️ **2026-08-10 の AA-D-22 / AA-D-23 で
  「MFA 必須 + 監査記録 + SuperAdmin の複数運用」が消え、担保はレート制限 1 本だけになった** —
  **この既定のまま進めるかをユーザーに再確認する必要がある** (FE-Q7 の回答を待つ)。**選択肢**: ①Vercel の固定 egress IP 機能を使う
