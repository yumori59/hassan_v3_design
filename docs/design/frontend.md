...
| **A-2** | **§9 / §11.1 / §11.3**: ①契約内管理者限定の 3 画面は**導線を出さない**が**判定の正は BE (403)** ②導線判定に使うロールの出所は**サインイン応答 → セッション**の 1 経路に固定 (§5.2.2。**応答に `role` を追加する是正要求は §16.2-6**) ③**社内管理者の画面を `(admin)` グループとして本増分に含める** (§11.3。[auth.md](auth.md) §6.2 の例外 2 件に対応。**画面の実数は §11.1 の表が正** — 2026-08-25 に契約管理 2 画面が増えた = AA-D-26) | **WAF の IP 許可リストが FE-D と両立しない**点は §11.3.2 で未解決として起票 (FE-Q7 / §16.2-7) |
