...
|---|---|---|---|---|---|
| AL-1 | 5xx エラー率 | 5 分間で 5% 超 | **初期実装 (⑧)** | **ALB の標準メトリクス** (`HTTPCode_Target_5XX_Count` / `RequestCount`)。2026-09-09 に確定 (下記の仮定解消) — アプリ側のメトリクス実装を待たずに成立する | 開発チーム (Slack) |
| AL-2 | LLM 失敗率 (F-1〜F-5 の合計) | 15 分間で 10% 超 | **初期実装 (⑧)** | §4.3 の失敗 5 分類のメトリクス (EMF。下記の名前空間・メトリクス名) の合計 | 開発チーム |
| AL-3 | **切り詰め (F-1) の発生** | 1 件でも発生 (初期は全件通知し、頻度を見て調整) | **初期実装 (⑧)** | §4.3 の F-1 (EMF: `llm.truncated`)。**`stop_reason` は ④ で初期実装**なので前提が揃う | 開発チーム |
| AL-4 | 日次コストの急増 | 前日比 200% 超 または 日次絶対額のしきい値超 | **v2 併用期間中 (⑦)** | コスト算出 (O-H の単価テーブル) と日次の集計メトリクス — **どちらも ⑦** | 開発チーム + 管理者 |
| AL-5 | 安全弁の発火 (§4.4) | 1 時間で N 件超 | **初期実装 (⑧)** | §4.3 の F-4 (打ち切り。EMF: `llm.timeout`) のメトリクス。**安全弁そのものは ⑤ で初期実装**なので、発火を観測できないと打ち切りの妥当性を判断できない | 開発チーム |
| AL-6 | ECS タスクの異常終了 / ヘルスチェック失敗 | 発生時 | **初期実装 (⑧ と同時期)** | **アプリのメトリクス基盤に依存しない** — ECS / ALB の CloudWatch 標準メトリクスで成立する (下記の注) | 開発チーム |
| AL-7 | **認証エンドポイントのレート制限の発動 (F-6) のスパイク** | 5 分間で N 件超 (初期値は実データで調整) | **初期実装 (⑧)** | §4.3 の F-6 (セキュリティイベント。EMF: `auth.rate_limit_triggered`) のメトリクス | 開発チーム。**単発の 429 は通常運用でも起きる**ため件数のスパイクを見る ([auth.md](auth.md) §6.11-4) |
...
- ~~メトリクスの実装手段~~ — **解消 (2026-09-09 ユーザー回答)**。方式 **B (アプリが EMF で直接出す)** に確定した
  (issue `hassan-v3-infra`#10 の差し戻し)。契約は「名前空間 + メトリクス名」になる:

  | 分類 | メトリクス名 | 対応するアラーム |
  |---|---|---|
  | F-1 (出力の切り詰め) | `llm.truncated` | AL-3 |
  | F-2 (JSON パース失敗) | `llm.json_parse_failed` | AL-2 (合計に含む) |
  | F-3 (ツール引数の不整合) | `llm.tool_args_invalid` | AL-2 (合計に含む) |
  | F-4 (タイムアウト / 打ち切り) | `llm.timeout` | AL-5 |
  | F-5 (SSE の異常終了) | `sse.abnormal_close` | AL-2 (合計に含む) |
  | F-6 (レート制限の発動) | `auth.rate_limit_triggered` | AL-7 |

  **名前空間は `hassan-v3/<env>`** (`env` は `dev` / `staging` / `prod`。preview (PR 単位) は
  `APP_ENV=dev` で動くため名前空間は共有し、ディメンションでスキーマ / PR を区別する必要が生じたら
  そのとき追加する)。**AL-1 (5xx エラー率) だけは ALB の標準メトリクスで成立する**ため EMF 化しない
  (アプリのメトリクス基盤より前に動く利点を活かす)。
