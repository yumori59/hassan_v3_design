...
> **この索引の目的**: `docs/design/` は 18 ファイル・約 12,000 行ある。
...
**この 3 つを読まずに書くと、層違反・所有者条件の欠落・ステータスコードの不統一が確実に出る**。
...
| ファイル | 主題 | 実装時に読む頻度 |
|---|---|---|
| [architecture.md](architecture.md) | 層構造・依存規則・トランザクション境界 | **高** (§3 は毎回) |
...
| [API/*.md](API/README.md) (7 ファイル) | ドメインごとの入出力仕様 | 担当ドメインのみ |
