...
- **社内管理者のロールによる制限の範囲は「契約管理 4 本」だけ** (**2026-08-26 のオーナー判断**)。
  社内管理者系統のうち、**ロール判定 (403) を課すのは上の例外 2 件目 (契約管理 4 本) のみ**で、
  **例外 1 件目の①群 (ロック解除・一般アカウントの MFA リセット・アカウント検索・管理者一覧・自分の情報) は
  `admin` ロールでも実行できる**。エンドポイント単位の一覧は
  [API/auth-accounts.md](API/auth-accounts.md) §2.4 が正。

  | 操作の性質 | ロール | 理由 |
  |---|---|---|
  | **契約を作る・変える** (契約管理 4 本) | **SuperAdmin 限定** | **課金に直結し不可逆**な商用上の意思決定であり、v2 自身の「Admin: Read 機能を利用可能」宣言に照らして書き込みを許さない |
...
1. v3 が新規に作る**機能テーブル**は、**所有者列を 1 段で直接持つ** (`NOT NULL` + FK)。
   ...
   **`contract_id` が全機能テーブルで必須**である一方、**`account_id` は個人境界のものだけが持つ**。
   ...
   実際の適用 (43 テーブルの内訳 = 個人境界 34 / 契約境界 9) は
