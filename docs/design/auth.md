...
| 4 | **社内管理者のロックアウト** (回復経路そのものが使えなくなる) | **サインイン失敗によるロックは発生しない** — §6.2 の決定により社内管理者にロック列を持たない。手動ロック API の対象も `accounts` のみ。**ただし MFA 必須化により「MFA デバイスの紛失」が新しい到達不能経路になる** → 他の SuperAdmin による MFA リセットで回復する (§6.2)。**SuperAdmin が 1 名でデバイスを失った場合のみ運用手順の対象** (§10.2 R-3) | **(2026-08-10 補足: AA-D-22 で MFA も課さないため、社内管理者は締め出しの経路自体を持たない)**
