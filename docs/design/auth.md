...
**帰結**: **利用者の打ち間違いと障害が区別できない**。さらに §6.2 が MFA を社内管理者の主たる防御に
据えたため、**MFA の総当たりが「5xx の山」として現れ O-4 / O-7 のアラートが意味を失う**。
...
**v3 での対応は §6.2 の成立条件 1 + §6.11-3 の対象②**。
...
- **含める範囲は「解除 + 到達に必要な最小の付随機構」** — 解除 API だけでは
  ...
  | **MFA の登録・検証** (TOTP 生成 / 検証) | `同:231-232` (ユーザー側が移植元) | **MFA 必須の帰結** (下記)。未検証状態で到達できる唯一のルート群として §6.7 のホワイトリストに載せる |
  | **社内管理者自身の MFA のリセット** (デバイス紛失時) | `同:83` (ユーザー側の `POST /accounts/mfa/reset` が移植元) | 実行者は **SuperAdmin のみ** (下記) |
  ...
  | 管理者アカウントの初期投入 | v2 は `同:206` (`CreateAdminAccount`) + `:199-200` (パスワード登録。公開) | **v3 では API を作らず、移行スクリプトによる投入とする** (`admin_accounts` の作成 API を持つと社内管理者機能が実質全部入りになるため)。**投入直後は MFA 未登録**なので、**初回サインイン後に TOTP 登録を強制する** (登録するまで解除 API に到達できない)。**初期パスワードの出所・窓の閉じ方は下記「初回登録の窓を閉じる」が SSOT**。運用手順は [operations.md](operations.md) |

  それ以外の管理者機能 (社内向け一覧・会社管理・利用状況閲覧) は**引き続き対象外**
- **社内管理者にサインイン失敗ロックを設けない**。代わりに **MFA を必須にする**
  (ユーザー決定 2026-07-30)。
  §5-4 は「v2 の `AdminAuthRequiredMiddleware` にロック判定が無い」ことを欠陥として挙げたが、
  **v3 での回答はロック機構の追加ではなく MFA の必須化**とする。

  > ### この判断の成立条件 (2026-07-31 追加。**条件が落ちたら判断ごと無効**)
  >
  > 「ロックを設けない」が安全なのは **MFA が有限回で破れないこと**が前提である。したがって次の 2 つを
  > **本決定の成立条件**とする — 実装リポで条件が落ちると「**ロックも無い・試行上限も無い**」状態が
  > *設計どおり*として成立してしまう:
  >
  > 1. **MFA 検証の試行回数に上限を設けること** (§6.11-3 の対象②)。
  >    根拠となる v2 の実測: **MFA 検証の失敗はロックカウンタを一切増やさない** —
  >    失敗回数の加算 SQL `UpdateFailedSignInAttempts` の非テスト呼び出し元は
  >    `hassan-v2-backend/usecase/account/sign_in.go:91` の**1 箇所だけ**で、そこは
  >    **パスワード検証失敗の枝**である (`usecase/mfa/verify_totp.go:56`〜`:60` は失敗時に activity log のみ)。
  >    上限が無ければ **`/admin/signin` でパスワードを通した攻撃者が MFA 未検証トークン 1 本で
  >    `POST /admin/mfa/totp/verify` を無制限に叩ける** — TOTP は 6 桁 = 探索空間 10^6 で、
  >    時刻ずれ許容により同時に有効なコードが複数ある実装が通例のため、並列試行で現実的な時間に収まる。
  >    突破の報酬は**全契約横断のロック解除**と (SuperAdmin なら) **他管理者の MFA リセット**であり、
  >    v3 で最も強い権限である
  > 2. **MFA コード不一致を 401 で返すこと** (§6.6。v2 は 500 = §5-12)。
  >    500 のままだと総当たりが「5xx の山」として現れ、**O-4 / O-7 のアラートが意味を失う**
  >    ([observability.md](observability.md) が「握り潰すと総当たり・パスワードスプレーの検知手段が無くなる」と
  >    書いた事象そのもの)
  >
  > **しきい値の関係**: §10.2 R-4 の「ロックしきい値 < レート制限しきい値」に
  > **第 3 のしきい値 (MFA 試行上限)** が加わる。3 者の関係は
  > **「ロックしきい値 < MFA 試行上限 < 未認証レート制限しきい値」**とする —
  > MFA 試行上限がロックより小さいと、パスワードを知らない攻撃者が MFA を叩くだけで
  > 正規利用者を締め出せる (新しい DoS 経路になる)。
  > **一般ユーザー側は `failed_sign_in_attempts` とは別カウンタで数える**
  > (同一にすると MFA の打ち間違いがサインインロックを引き起こし、§6.9 の回復経路が過剰に発動する)。

  > ### 初回登録の窓を閉じる (2026-07-31 追加。**MFA が有効になるまでの窓の防御**)
  >
  > MFA 必須化は「MFA が有効になってから」効く。**移行スクリプトによる投入から各管理者の初回 TOTP 登録が
  > 完了するまでの窓は、防御がパスワード 1 要素しかない**。この窓には次の 3 つが同時に成立する:
  >
  > | # | 事実 | 出典 |
  > |---|---|---|
  > | 1 | 社内管理者に**ロックが無い** (本決定) | 本節 |
  > | 2 | **MFA 未検証トークンで `POST /admin/mfa/totp/generate` に到達できる** (§6.7 の 4 系統目)。v2 の移植元は `is_verified == false` の設定を**削除して再生成を許す**ため、**パスワードを知る者が自分の authenticator を登録できる** | `hassan-v2-backend/usecase/mfa/create_totp.go:41`〜`:46` |
  > | 3 | 登録されると正規の管理者は `TotpMfaAlreadyRegistered` で登録できず、回復には**他の SuperAdmin による MFA リセット**が必要 (SuperAdmin が 1 名なら製品内で回復不能) | `同:36`〜`:38` / 本節下部 |
  >
  > つまり**攻撃者が締め出し (DoS) と乗っ取りを同時に成立させられる**。そのうえで、
  > **v2 の管理者パスワードは `math/rand` ベースの登録トークン経由で設定された**ものであり
  > (`hassan-v2-backend/usecase/admin_account/create_admin_account.go:90` が
  > `util.GenerateSecureToken()` = `util/util.go:30`〜`:36`。**import は `math/rand` のみ** — `同:3`〜`:5`。
  > 有効期限 1 週間でメール送信 — `同:95`〜`:100`)、**§5-8 が「より深刻」と認定した経路そのもの**である。
  >
  > **決定 (4 点)**:
  >
  > 1. **v2 の管理者パスワードを v3 へ移送しない**。§6.8-2 が JWT 鍵について採ったのと同じ論法
  >    (**露出済みの値を安全な保管先に入れても意味がない**)。移行スクリプトは投入時に
  >    **`crypto/rand` で一時パスワードを生成** (§6.10-1) し、**帯域外 (口頭 / 既存の社内認証済みチャネル) で配布する**
  > 2. **窓に期限を持たせる**。一時パスワードに**有効期限を持たせ、期限内に初回サインイン + TOTP 登録を
  >    完了しないアカウントは無効化する** (再発行は移行スクリプト / 運用手順の対象)。
  >    期限の具体値は [operations.md](operations.md) の運用手順で決める (§10.2 R-7)
  > 3. **未登録アカウントの `/admin/signin` も §6.11-3 の対象②に含める** (上記「成立条件」1 と同時に閉じる)
  > 4. **初回 TOTP 登録の完了を監査記録に残す** (O-6)。締め出しが起きたときに
  >    「いつ誰が authenticator を登録したか」が分からないと調査できない
  >
  > **併せて明示する非対称**: v2 の全契約横断ロック解除は **Admin ロールでも実行できる**
  > (`hassan-v2-backend/router/router.go:211` は `.Use()` 配下だが `CheckSuperAdminRole` を持たない)。
  > 一方 MFA リセットは SuperAdmin 限定である。**権限の強さは「全契約のロック解除」≧「他管理者の MFA リセット」**
  > なので、**v3 はロック解除も SuperAdmin 限定にする** (v2 と同じにする案は却下 —
  > 弱い方に強い権限が付く非対称に設計上の理由が無い)。

  **ロックを設けない理由**: 社内管理者は少人数であり、**全員がロックされたときの回復手段を
  別に用意する必要が生じる** — 回復経路 (§6.9) を作るために導入した機構が、
  新しいロックアウト経路を生む。器も v2 に無い:

  | 事実 | 出典 |
  |---|---|
  | `admin_accounts` に `last_locked_at` / `failed_sign_in_attempts` の**カラムが存在しない** | `hassan-v2-backend/db/schema.sql:52-64` |
  | 管理者アカウントを対象にした解除クエリも v2 に無い (`db/queries/*.sql` の `last_locked_at` は全て `accounts` 向け) | `hassan-v2-backend/db/queries/account.sql:56-81` |

  **MFA 必須は v2 の機構を流用できない — 新規実装になる** (この点を過小評価しない):

  | 必要なもの | v2 の状況 | 出典 |
  |---|---|---|
  | MFA 設定の器 | `account_mfa_configs.account_id` は **`accounts` への FK** なので `admin_accounts` に使えない。**`admin_mfa_configs` 相当を新設する** | `hassan-v2-backend/db/schema.sql:68-77` |
  | トークンのクレーム | 管理者トークンは `uid` + `exp` のみ。**`mfa_verified` を追加する** (ユーザー側 `JwtClaims` と同じ考え方。§1.2) | `hassan-v2-backend/auth/client.go:92-95` |
  | ミドルウェアの判定 | `AdminAuthRequiredMiddleware` に MFA 判定が無い。**「MFA 未検証なら MFA 系ルート以外を拒否」を追加する** — 判定は §6.7 のホワイトリスト方式で行い、v2 のパス前方一致 (§1.4) を持ち込まない | `hassan-v2-backend/auth/middleware.go:88-131` |
  | 登録・検証フロー | ユーザー側の TOTP 実装が移植元になる (`POST /mfa/totp/generate` / `verify`)。**`reset` は移植元に含めない** — v2 の `POST /mfa/totp/reset` は**現在の TOTP コードを要求する**ため紛失時に使えない自己リセットであり (`hassan-v2-backend/usecase/mfa/reset_totp.go:35`〜`:38`)、上表の「MFA のリセット」(管理者実行) とは**別機能**である。同じ「MFA リセット」の語で 2 つを指さない。**移植時に §5-12 / §5-13 を引き継がないこと** (下記) | `hassan-v2-backend/router/router.go:231-233` |

  > **移植元に埋まっている欠陥 (引き継がない。2026-07-31 追加)** — §5 に **5-12 / 5-13** として登録した:
  >
  > | # | 欠陥 | 出典 | v3 の扱い |
  > |---|---|---|---|
  > | §5-12 | **MFA コード不一致が 500 になる** — UseCase は `apperror.TotpCodeNotMatch()` を返すが Controller が全エラーを `internalServerError` へ流す | `hassan-v2-backend/controller/mfa.go:80`〜`:83` (`ResetTotp` も `:113`〜`:116`) | **401 + `CodedError`** (§6.6)。上記「成立条件」2 |
  > | §5-13 | **MFA 検証失敗が失敗回数に加算されない** | 加算経路は `hassan-v2-backend/usecase/account/sign_in.go:91` の 1 箇所のみ | **§6.11-3 の対象②で試行上限を掛ける**。上記「成立条件」1 |

  **一般ユーザーとの違い**: 一般ユーザーの MFA 要否は**会社単位の設定** (`companies.mfa_type`。§1.4) だが、
  **社内管理者は設定で無効化できない固定の必須**とする (コード上の分岐にしない)。

  **MFA デバイス紛失時の回復** (これを決めないと §6.9 と同じ「回復不能」を作る):
  **他の社内管理者が MFA をリセットする**。実行者は **SuperAdmin ロールに限る**
  (`hassan-v2-backend/entity/admin_auth_role.go:12` の `AdminAuthRoleSuperAdmin`、
  判定は `hassan-v2-backend/auth/middleware.go:153-163` の `CheckSuperAdminRole`。
  **ただし v2 は権限不足に 401 を返しており、v3 は 403 にする** — §5-3 / §6.6)。
  v2 にはユーザー向けの同等機構がある (`POST /accounts/mfa/reset` = 契約管理者が実行、
  `hassan-v2-backend/router/router.go:83`)。
  **SuperAdmin が 1 名で、その MFA デバイスを失った場合**は製品内で回復できない —
  この 1 ケースのみ**運用手順 (`docs/design/operations.md`) の対象**とし、
  **SuperAdmin を 2 名以上運用することを前提として明記する** (§10.2 R-3 / operations への申し送り)。

  **追加の層 (MFA と併用する)**: ①`POST /admin/signin` を §6.11-3 のレート制限対象に含める /
  ②サインイン試行と MFA 検証の成否を監査記録に残す (O-6) /
  ...
  MFA を主たる防御とし、IP 制限は多層防御として併用する (どちらかで代替しない)
...
1. v3 が新規に作る**機能テーブル**は、**所有者列を 1 段で直接持つ** (`NOT NULL` + FK)。
   ...
   **`contract_id` が全機能テーブルで必須**である一方、**`account_id` は個人境界のものだけが持つ**。
   ...
   実際の適用 (40 テーブルの内訳 = 個人境界 32 / 契約境界 8) は
...
3. **機械検査**: **除外リスト**に無いテーブルが **`contract_id` を持たない**状態を CI で検出する。
   ...
   **本節に件数を転記しない** (2026-07-31 改訂 — DR-9。この件数は 2026-07-30 に 9 件・
   2026-07-31 に 8 件・同日 `admin_mfa_configs` の追加で 9 件と 2 日で 3 回動いた集合であり、
...
| **`admin_mfa_configs`** | **社内管理者の MFA 設定** (v3 新設。§6.2)。`admin_accounts` に属し**契約を横断する運用主体の設定**なので所有者列を持たない = 本検査の例外 (**2026-07-31 追加**。定義は [data-model.md](data-model.md) §4.2) |
...
- **許可リストによる例外化** (ユーザー決定 2026-07-30。§6.10-3 の `math/rand` 検査と同じ運用):
  ...
  **許可される 7 種**:
  ...
  > | 2 | **許可リストに「呼び出しを許す系統」列を必須にする** — 値は §6.7 の 4 系統のいずれか。**§6.7 の系統検査と突き合わせて機械判定する** (⑦は書き込みであり、系統は `X-Admin-Token` の有無で判定できる)。**③に呼び出し元を書かせる案を却下した理由 (レコードを返す経路が残る) は⑦には当てはまらない** |
...
| **MFA コードの試行超過** | **429** | `CodedError` | **2026-07-31 追加**。§6.11-3 の対象②。**§6.2 の成立条件 1** (ロックを設けない判断が成立するための必須条件) |
...
- `r.Group()` の直後に `.Use(AuthRequiredMiddleware(...))` を書き、
  **公開エンドポイントだけを別グループに切り出す**。§1.7 の順序依存 (`:199-201`) を
  グループの分離で表現する
- **v3 は 4 系統を持つ** (§6.2 の例外により社内管理者系と、その MFA 未検証状態が入ったため)。
  **ホワイトリストは「パス → 要求する認証系統」の宣言**とし、
  ...
  | **社内管理者認証** | `AdminAuthRequiredMiddleware` 相当 (**MFA 検証済みを要求**) | **主体を明示する (2026-07-31 改訂)**: **社内管理者による全契約横断のロック解除** / **社内管理者自身の MFA リセット (SuperAdmin 限定)** / **社内管理者による一般アカウントの MFA リセット** (§6.2)。**契約内管理者による同名の操作はここに載せない** (上のユーザー認証行) |
  | **社内管理者認証 (MFA 未検証で可)** | `AdminAuthRequiredMiddleware` 相当 (MFA 判定を免除) | **社内管理者の MFA の登録・検証のみ** (§6.2)。**この系統に載せるルートをホワイトリストで限定する** — v2 のパス前方一致 (§1.4) を持ち込まない。**ユーザー側にも同型の「MFA 未検証で到達可」なルート (`POST /mfa/totp/generate` / `verify`) がある** — こちらは「ユーザー認証」系統の内側で §6.6 の MFA ゲートを免除するホワイトリストとして表現する (系統は増やさない) |
...
| 4 | **社内管理者のロックアウト** (回復経路そのものが使えなくなる) | **サインイン失敗によるロックは発生しない** — §6.2 の決定により社内管理者にロック列を持たない。手動ロック API の対象も `accounts` のみ。**ただし MFA 必須化により「MFA デバイスの紛失」が新しい到達不能経路になる** → 他の SuperAdmin による MFA リセットで回復する (§6.2)。**SuperAdmin が 1 名でデバイスを失った場合のみ運用手順の対象** (§10.2 R-3) |
...
3. **ロックでは防げない濫用に対する制限を設ける** — アカウント単位のロックは
   ...
   **同一アドレスへのリセットメール連投**を防がない。
   ...
   | **対象② (認証済み。2026-07-31 追加)** | **資格情報・ワンタイムコードを検証する認証済みエンドポイント** — `POST /mfa/totp/verify` / `POST /admin/mfa/totp/verify` / **パスワード再入力を伴う操作** (`PUT /accounts/me/password` の `old_password` / `PUT /accounts/me/email`)。**キーは `account_id` / `admin_account_id` + エンドポイント** (認証済みなので主体が特定でき、IP を使う必要がない)。**この対象が §6.2 の「ロックを設けない」判断の成立条件である** — 旧版は対象を「未認証で叩けるエンドポイント」と定義しており、**認証済みの `/mfa/totp/verify` が構文上 1 つも入らなかった** (§5-13 と合わせて MFA が無制限に試行できる状態だった) |
...
| ID | 状態 | 対応 AC | 回答 |
|---|---|---|---|
| A-1 認証方式 | **回答** | **AC-1.1** / **AC-1.5** | §6.1。v2 の `X-Token` + HS256 JWT を踏襲。JWT ライブラリのみ `golang-jwt/jwt/v5` へ差し替え。**方式の踏襲に伴う条件を §6.8〜§6.11 で確定** (AC-1.5): 署名鍵の保管と複数鍵ローテーション (§6.8。保管方式の SSOT は [architecture.md](architecture.md) §5 D-5 / AC-3.1)、有効期間 7 日据え置き + **手動ロック API による即時失効** (§6.9)、認証フローの乱数生成を `crypto/rand` に統一 (§6.10)、認証エンドポイントの応答・レート制限・観測 (§6.11)、**社内管理者の MFA 必須化** (§6.2。`admin_mfa_configs` の新設・管理者トークンへの `mfa_verified` 追加・ミドルウェア判定・登録/検証/リセットのフロー) |
| A-2 ロールと適用範囲 | **回答** | **AC-1.1** | §6.2。本増分は一般ユーザー (`AuthRoleUser`) が原則。**例外として社内管理者 (`X-Admin-Token`) 系を含む** — 全契約横断のロック解除・サインイン・**MFA 登録/検証/リセット** (§6.9 の回復経路を成立させるため)。**社内管理者は MFA 必須** (一般ユーザーは会社単位で任意)。`AuthRoleConsultant` は対象外。**適用範囲の機械強制は §6.7 の 4 系統ホワイトリスト** |
...
| A-7 共有・公開 | **回答** | — (対応 AC なし。**要件側に AC が無いこと自体が課題** → §10.2 R-8) | **§6.12 が SSOT** (2026-07-31 新設)。(a) `sharing_settings` は引き継がず既存値を `visibility` の初期値に使う (b) **アイデアボードは v2 の契約内共有と 3 段ロールを増分 1 で引き継ぐ** (c) テーマ・アセットの `visibility` 書き込み API は増分 2 (列は増分 1) (d) 契約外公開 (`open`) は持たない。**旧記述「対象外 (暫定)。[architecture.md](architecture.md) §5 と同じ判断」は撤回した** — 参照先の architecture.md が当該判断を取り下げており (同 `:791`)、**A-7 の判断を持つ文書が 1 つも無い循環参照になっていた** |
...
| 項目 | 状態 |
|---|---|
| §6.3 の例外テーブル列挙 | **確定** — v3 が持つアイデンティティ基盤テーブルは [data-model.md](data-model.md) §4.2 が定義 (`admin_mfa_configs` を含む)。**件数の転記は廃止**し同 §4.1.2 の 2 表を SSOT にした |
  ...
  **帰結として残るリスクを明示する**:
  ...
  - **ただし D-7 の併用期間中 (v2 と v3 が本番で並走する間) は v2 側の経路が残る**。
    ...
    **v2 で `math/rand` 経由のリセットトークンによって設定されたパスワードがそのまま v3 に入る**。
    社内管理者については §6.2 の「初回登録の窓を閉じる」1 で**移送しない**と決めたが、
...
| **R-3** | [API/auth-accounts.md](API/auth-accounts.md) (`plan.md` Task-3i) | v3 の認証系エンドポイントの**入出力仕様**。**必ず含めること**: ①メンバー一覧でロック状態を返す ②§6.4 の許可リストに載せる認証系クエリの確定 ③**社内管理者の MFA フロー** (`admin_mfa_configs` のスキーマ・登録/検証/リセット・初回登録強制) ④MFA リセットの実行者を SuperAdmin に限る (403) | **実施済み (2026-07-31)** — [API/auth-accounts.md](API/auth-accounts.md) が 37 エンドポイントで確定。**③のスキーマは [data-model.md](data-model.md) §4.2 に `admin_mfa_configs` として定義済み** (同日追加)。宛先は `settings.md` から `auth-accounts.md` に移った |
...
| 同 | **R-AA-9** | 系統表にユーザー側の「MFA 未検証で到達可」が無い非対称 | **実施済み** (§6.7 の 4 系統表) |
...
**§6.12 (c) を「増分 2」から「増分 1」へ改訂した**ことによる波及。**反映済みは [data-model.md](data-model.md) DM-9 / [API/themes.md](API/themes.md) TM-2・D-TH-5 / [API/assets.md](API/assets.md) AS-M2 / [API/settings.md](API/settings.md) D-ST-3 の 4 書**。**下記 2 書は別セッションが編集中のため未反映**であり、要求として残す:

| # | 対象 | 要求 | 状態 |
|---|---|---|---|
| **R-9** | [API/README.md](API/README.md) **D-API-8'** / §3 の `scope` 記述 / §5 の A-7 行 / §5 API-Q3 / 「増分 2 の作業単位」 | **`scope=contract` を増分 1 で有効化する**。「増分 1 は `mine` のみ・`contract` は 400」を撤回し、**増分 2 に残るのはテーマメンバー機能のみ**にする。A-7 行は「回答」に更新し §6.12 を参照する。**API-Q3 は A-7 の判断部分を閉じ、テーマメンバーの権限差 (TH-Q5) の要件確認のみに縮小する** | **未対応** (並行編集) |
| **R-12** | [API/README.md](API/README.md) §3 の総覧 / [frontend.md](frontend.md) §11.1 | **`GET /ideas/csv` の追加に伴う反映** (2026-07-31 の TH-Q1 決着 = v2 の CSV エクスポートを引き継ぐ。仕様は [API/idea-boards.md](API/idea-boards.md) §2.4)。①README §3 の総覧に 1 本追加し**合計値を更新する** (idea-boards の `/ideas` 系は 3 本 → **4 本**) ②frontend.md にエクスポートボタンの配置を追加する (**v2 は発散画面から落とす形。テーマ一覧のヘッダではない** — 2026-07-30 のプロトタイプにボタンが無いのは「テーマ管理ヘッダ」であって、アイデア一覧のエクスポート自体が不要という根拠にはならない) | **未対応** (並行編集) |
| **R-11** | [API/auth-accounts.md](API/auth-accounts.md) **AA-D-15** / §4 の A-7 行 (`:686` 付近) | **根拠の差し替えのみ** (判断内容は妥当なので変えない)。「`GET /contracts/me` から `sharing_settings` を落とす」の根拠が「**§7 の『本増分では共有機能を持たない』と整合させた**」になっているが、**その記述は §6.12 の新設で撤回された**。新しい根拠は「**契約単位の共有既定は [API/settings.md](API/settings.md) の `/settings/workspace` が持ち、per-resource の可視性は各リソースが持つため、契約情報の応答に共有設定を載せる必要が無い**」。§4 の A-7 行の「本書は共有機能を持たない」も同様に書き換える | **未対応** (並行編集) |
