...
templates/backend-repo/.claude/rules/05-architecture-coding-rules.md
templates/backend-repo/CLAUDE.md.tmpl
templates/backend-repo/STRUCTURE.md
templates/backend-repo/layering-scopes.yml
templates/backend-repo/prompts/agents.yaml
...
templates/backend-repo/common/auth/.gitkeep
templates/backend-repo/common/config/.gitkeep
templates/backend-repo/common/constants/.gitkeep
templates/backend-repo/common/di/.gitkeep
templates/backend-repo/common/logger/.gitkeep
templates/backend-repo/common/router/.gitkeep
templates/backend-repo/controller/.gitkeep
templates/backend-repo/controller/dto/.gitkeep
templates/backend-repo/db/queries/asset/.gitkeep
templates/backend-repo/db/queries/conversation/.gitkeep
templates/backend-repo/db/queries/idea/.gitkeep
templates/backend-repo/db/queries/plan/.gitkeep
templates/backend-repo/db/queries/theme/.gitkeep
templates/backend-repo/db/rdb/.gitkeep
templates/backend-repo/entity/.gitkeep
templates/backend-repo/entity/toolresult/.gitkeep
templates/backend-repo/env/.gitkeep
templates/backend-repo/gateway/anthropic/.gitkeep
templates/backend-repo/gateway/exa/.gitkeep
templates/backend-repo/gateway/gemini/.gitkeep
templates/backend-repo/gateway/mail/.gitkeep
templates/backend-repo/gateway/s3/.gitkeep
templates/backend-repo/prompts/asset/.gitkeep
templates/backend-repo/prompts/conversation/.gitkeep
templates/backend-repo/prompts/idea/.gitkeep
templates/backend-repo/prompts/plan/.gitkeep
templates/backend-repo/repository/asset/.gitkeep
templates/backend-repo/repository/conversation/.gitkeep
templates/backend-repo/repository/idea/.gitkeep
templates/backend-repo/repository/plan/.gitkeep
templates/backend-repo/repository/theme/.gitkeep
templates/backend-repo/service/asset/.gitkeep
templates/backend-repo/service/conversation/.gitkeep
templates/backend-repo/service/idea/.gitkeep
templates/backend-repo/service/plan/.gitkeep
templates/backend-repo/service/theme/.gitkeep
templates/backend-repo/testdata/golden/toolresult/.gitkeep
templates/backend-repo/usecase/asset/.gitkeep
templates/backend-repo/usecase/conversation/.gitkeep
templates/backend-repo/usecase/idea/.gitkeep
templates/backend-repo/usecase/plan/.gitkeep
templates/backend-repo/usecase/theme/.gitkeep
```
...
- `templates/backend-repo/layering-scopes.yml:52` が「`architecture.md` §3.5.1 の `common/` の決定」を参照するが、
  **その決定は push される `architecture.md` に存在しない** (重大 2 の状態のまま)
- `templates/backend-repo/STRUCTURE.md:111` の「v2 実測 0 件」(事実誤り) と
...
- 対象ファイルは 1 巡目の 106 → **108** (`+2` = `templates/backend-repo/.claude/rules/05-architecture-coding-rules.md`
...
| # | 主張 (出典) | 照合コマンド | 結果 |
|---|---|---|---|
| 1 | **v2 実測: `usecase/` → `controller/dto` の import は 0 件** (`templates/backend-repo/STRUCTURE.md:111`) | `grep -rl "controller/dto" usecase/ \| wc -l` → **29** / 非テスト **27** / `usecase/asset/list_assets.go:7` に実在 | **誤り (重大 1 継続)** |
...
| 5 | `L1-usecase-no-controller` の deny が `<module-path>/controller` (前方一致で `controller/dto` を含む) | `templates/backend-repo/.golangci.yml:150-156` | **一致** (重大 1 の帰結 2 が成立) |
...
| 7 | `env/<env>.env` のキー集合を CI で照合する (`05-architecture-coding-rules.md:247`) | `grep -n "env/\|\.env\|check-env\|secret" templates/backend-repo/.github/workflows/ci.yml` → **0 件** | **機構なし (重大 3 継続)** |
...
|---|---|---|
| **重大 1** 「v2 実測 0 件」が事実誤り | **未解消** | `templates/backend-repo/STRUCTURE.md:111` に「0 件」が残存 (照合 #1 で再現)。`architecture.md` 側の訂正は**未コミット** (`:180`〜`:198`)。移植分の depguard 扱いも `templates/backend-repo/.golangci.yml:150-156` は未変更 |
...
| **重大 3** env キー集合照合 CI が無い (D-5) | **未解消** | `templates/backend-repo/.github/workflows/ci.yml` に env 関連ステップ 0 件 (照合 #7)。要求元は `4afd59b` で `CLAUDE.md.tmpl:257` → **`05-architecture-coding-rules.md:247` に移動しただけ** |
...
| **中 1** `agents.yaml` の `tools: []` が無言で緑 (BE-9) | **未解消** | `templates/backend-repo/prompts/agents.yaml` は `4afd59b` で未変更 |
...
- **重大 1**: `templates/backend-repo/.golangci.yml:150-156` を実測し、`deny.pkg: "<module-path>/controller"` が
...
| 場所 | 記述 |
|---|---|
| `templates/backend-repo/CLAUDE.md.tmpl:40`〜`:41` (本コミットで新規) | 「要点だけ: Clean Architecture 4 層 (**Controller → UseCase → Service → Repository**) + `entity/` + `gateway/` の計 6 パッケージ層」 |
| `templates/backend-repo/.claude/rules/05-architecture-coding-rules.md:70`〜`:71` | 「**`usecase` は `repository` / `gateway` の具体パッケージに依存しない**。「Controller → UseCase → Service → Repository」の直列図は**この意味で誤読になるので使わない**」 |
...
**事実**: `4afd59b` は `templates/backend-repo/.claude/agents/go-developer.md:99-102` に
...
を追加した。しかし `templates/backend-repo/.claude/agents/code-reviewer.md` には **`rules/05` への言及が 1 件も無い**
...
| 場所 | 記述 |
|---|---|
| `templates/backend-repo/CLAUDE.md.tmpl:57` | 「`.claude/rules/feedback_review_patterns.md` \| 頻出バグパターン (**SSOT**。設計リポと還流)」 |
| `templates/frontend-repo/CLAUDE.md.tmpl:92` | 同文 |
| `templates/infra-repo/CLAUDE.md.tmpl:67` | 同文 |
| `templates/backend-repo/.claude/agents/code-reviewer.md:139` | 「**`.claude/rules/feedback_review_patterns.md` (SSOT)** をチェックリストとして必ず使う」 |
| `templates/frontend-repo/.claude/agents/frontend-reviewer.md:62` | 同旨 |
...
> `templates/backend-repo/CLAUDE.md.tmpl` \| `:31-45` (アーキテクチャ節) + `:47` (エラー規約) \|
...
**是正案**: 当該行の対象を `templates/backend-repo/.claude/rules/05-architecture-coding-rules.md` に付け替え、
...
- **中 3** → `templates/backend-repo/.claude/rules/05-architecture-coding-rules.md:244`〜`:247`
- **中 2 / 中 4** → `templates/backend-repo/STRUCTURE.md:9` / `:127-128` / `:197` / `:202-205`
...
3. **重大 3**: `templates/backend-repo/.github/workflows/ci.yml` に env キー集合検査の
