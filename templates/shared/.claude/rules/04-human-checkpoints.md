# Rule 04: 人間の承認点と、それを止める機構 (backend / frontend / infra 共通)

**AI 駆動でも人間が必ず判断する点**を一覧で確定し、**各点をどの機構が止めるか**を定める。
このファイルは 3 リポジトリ共通 (`templates/shared/` から各実装リポの `.claude/rules/` にコピーされる)。
...
| issue の粒度・DoD・リポ跨ぎのマージ順序 | rule `02-issue-granularity.md` |
...
|---|---|---|---|---|---|---|
| **H-1** | **PR のマージ** | S-10 | 3 リポすべて | ① CI 全ジョブ green ② PR 本文のレビュー結果要約に**重大ゼロ** ③ DoD チェックリストの充足 ④ 対象 AC-ID とテスト名の照合出力 ⑤ 依存 issue (他リポ) のマージ済み ⑥ H-2 / H-3 の該当宣言の有無 ⑦ H-5 該当 issue なら承認コメントの URL | **PR の approve レビュー + マージコミット** | ブランチ保護 (必須レビュー + 必須ステータスチェック + `main` への直接 push 禁止) §2.1 |
| **H-2** | **DB マイグレーションの適用** | S-10 後 (`deploy.yml` の `apply_migration` ジョブ) | backend | ① 適用される DDL 差分の全文 ② **破壊的変更の検出結果** (§2.2 の定義) ③ 適用先環境 ④ 後方互換か (旧イメージが動き続けるか) ⑤ ロールバック手順の有無 | **GitHub environment の承認履歴** (`prod-db` / `dev-db-destructive`) | GitHub Actions environment の required reviewers §2.2 |
| **H-3** | **Managed Agent の再発行** | S-10 後 (`apply_agent` ジョブ。**アプリのリリースより前**) | backend | ① prompt / tool schema の差分 ② **schema ↔ handler ↔ prompt の 3 者一致検査が green** ③ `Tools` の全置換で既存ツール (web_search 等) が落ちていないこと ④ Agent ID が変わることによる**進行中セッションの切断**の影響 | **GitHub environment の承認履歴** (`prod-agent`) | 同上 §2.3 |
| **H-4** | **本番環境への適用 (デプロイ)** | S-10 とは独立した判断 (手動起動) | backend (ECS) / frontend (Vercel) / infra (`terraform apply`) | ① 対象イメージタグ / コミット SHA ② その変更が dev で検証済みであること ③ H-2 / H-3 が先に完了していること ④ infra は `plan` 差分 (`destroy` / `replace` の有無) ⑤ ロールバック手段 | **GitHub environment の承認履歴** (`prod`) / Vercel の Promote 操作ログ / infra は `apply` 実行者本人 | `workflow_dispatch` + environment 承認 + `main` 限定 §2.4 |
| **H-5** | **着手前の計画承認** (**条件付き**) | S-2 (§1.2 の条件に該当する issue のみ) | 3 リポすべて | ① 変更計画 (触るファイル・層・追加するテーブル / エンドポイント) ② 設計書の該当節との対応 ③ 3 リポ跨ぎの場合はマージ順序 ④ 却下する場合は設計リポへ差し戻すかの判断 | **issue コメント** (承認者と日付を明記) | issue テンプレートの必須欄 + `needs-human` ラベルでの停止 + PR の DoD 欄 → **H-1 で検証** §2.5 |
...
|---|---|---|---|
| **dev** | **非破壊なら自動適用** / **破壊的変更は承認必須** (`dev-db-destructive`) | 自動 (dev の Agent ID は dev 専用。切断の影響は開発者のみ) | 自動 (`main` への push で継続デプロイ) |
...
| **③ 3 リポ跨ぎ** | issue テンプレートの「**依存 issue (他リポ)**」欄が埋まっている、または同一機能の issue が他リポに存在する |
...
`deploy.yml` を **`plan_migration` (検査・承認ルーティング) → `apply_migration` (適用)** の 2 ジョブに分ける。
...
**マイグレーション方式 (psqldef / golang-migrate) は設計リポで未確定** (`docs/design/architecture.md` の
...
| リポ | 機構 |
|---|---|
| **backend** | `deploy.yml` の `workflow_dispatch` (`environment: prod` を明示選択) + `release` ジョブの `environment: prod` (承認者必須) + **`main` 以外の ref からの prod 起動を最初のジョブで失敗させる** |
| **frontend** | Vercel の Production Branch を **`production`** に設定し、`main` は Preview にする。本番昇格は人間の Promote 操作 (または `production` への人間によるマージ)。`main` を Production Branch にしない |
...
1. **issue テンプレートの必須欄**「人間チェックポイントの該当有無」に選択肢
   「着手前の計画承認 (新規ドメイン / 設計書に無いパターン / 3 リポ跨ぎ)」を置く
...
| 承認点 | 機構 | 回避可能性 | 二重化 |
|---|---|---|---|
| H-1 マージ | ブランチ保護 (GitHub) | **回避不可** (サーバ側) | — |
...
**H-5 以外は GitHub 側の機構で回避不可能な形にする**。この表に「機構: なし」の行を作らないこと
...
- [ ] **Require status checks to pass before merging** を有効化し、**必須チェック名**を登録:
  - backend: `build / vet / test / lint`
  - frontend: `tsc / test / build / lint`
  - infra: `fmt / validate / lint` と `plan (dev)`
...
### 4.2 GitHub Environments — backend リポ

`deploy.yml` が参照する environment を作り、**承認者 (required reviewers) を設定する**。
...
# environment と承認者 (backend リポ。reviewers が空の prod* があれば設定漏れ)
for e in dev dev-db-destructive prod-db prod-agent prod; do
  echo "== $e"
...
| H-4 (infra) | `apply` の実行者が **PR に `apply` 後の出力 (適用結果の要約) をコメントする** — CI に記録が残らないため、これを記録手段とする |
