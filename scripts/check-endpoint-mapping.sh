...
#
# ⚠️ 多重ヒット検査つき (2026-08-10 追加。レビュー 重大 9 の是正)。
# `head -1` だけだと **同一パターンの 2 個目以降に旧値が残っても素通りする**。
# check-table-counts.sh は 2026-07-31 にこの穴を pick() の多重ヒット検査で塞いだが、
# 本スクリプト側は塞がれていなかった (実測: README の「429 を返す 11 本」が
# 2 箇所にあり、1 箇所だけ更新された状態が検出できなかった)。
# pick() はコマンド置換 (サブシェル) で呼ばれるため errors への加算が親に伝わらない。
# 一時ファイルに記録して最後に集計する。
MULTIHIT_LOG=$(mktemp)
...
# ⚠️ パターンを **§2 の見出し形 (`## 2. エンドポイント一覧 (合計 **N 本**)`) に限定する** (2026-08-10)。
# 旧パターン `(エンドポイント|合計|計) N (本|エンドポイント)` は「レート制限 計 10 本」「403 合計 16 本」など
# **意味の違う数え上げまで拾う**ため、pick() の多重ヒット検査が誤検知になる。
# 多重ヒット検査は「同じ主張の 2 個目に旧値が残る」を捕まえるものなので、パターンは主張ごとに一意にする。
aa_claim=$(pick '## 2\. エンドポイント一覧 \(合計 \*{0,2}[0-9]+ ?\*{0,2}本\*{0,2}\)' "$AA")
...
# ⚠️ パターンを §3.7 の見出し形に限定する (2026-08-10)。広いパターンだと「レート制限 N 本」の
# 別文脈 (§2.6 の未照合リスト・§4 の A-5 回答) を先に拾い、自称値の改ざんが素通りする (故障注入で確認)。
aa_429_claim=$(pick 'レート制限の対象 \(計 \*{0,2}[0-9]+ ?\*{0,2}本' "$AA")
expect "429 を返すエンドポイント実数 == auth-accounts.md の自称値" \
  "$aa_429" "$aa_429_claim" "R-AA-25"

# 「429 を返す N 本」から末尾の数値を採る (先頭から採ると "429" を拾う — pick() と同じ規約)。
# **括弧付きの表記 (`429 を返す (10 本)`) も拾う** — 2026-08-10 に README §2.5 が括弧付きで
# 旧値を保持したまま素通りしていた (レビュー 重大 9)。
rm_429=$(pick '429 を返す ?\(?\*{0,2}[0-9]+ ?\*{0,2}本' "$RM")
...
# ── ⑧ auth-accounts の 403 の散文値 (2026-08-10 追加 = レビュー 重大 10) ──
# §3.1 の「403 の N 本は契約内管理者限定」と README §3 の「同書の 403 は N 本ある」/
# 「認証・アカウント基盤の N 本は別勘定」が、§2 の実測 (403 を返す行数) と一致すること。
# 4 箇所が無検査で 9 / 10 / 10 とばらついていた。
aa_403_prose=$(pick '403 の ?\*{0,2}[0-9]+ ?\*{0,2}本は契約内管理者限定' "$AA")
expect "auth-accounts.md §3.1 の 403 の散文値 == §2 の実測" "$aa_403" "$aa_403_prose" "⑧"
rm_403_prose=$(pick '同書の 403 は ?\*{0,2}[0-9]+ ?\*{0,2}本ある' "$RM")
expect "README.md の「同書の 403 は N 本ある」== auth-accounts の実測" "$aa_403" "$rm_403_prose" "⑧"
