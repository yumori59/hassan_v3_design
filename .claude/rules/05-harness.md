...
| **API のエンドポイント件数・移植チェックリストの対応・custom tool 本数・403 本数・CSV 列数** (`check-endpoint-mapping`。2026-07-31 新設 → 検査は追記で伸びる。下の①〜⑨) | **エンドポイント設計そのものの妥当性** (パス・入出力・ステータスコード) |
...
⑧**auth-accounts の 403 の散文値 3 箇所 ↔ §2 の実測** (2026-08-10 追加 — 4 箇所が無検査で 9 / 10 / 10 と
ばらついていた。同時に `pick()` に多重ヒット検査を移植し「同一文言の 2 個目に旧値が残る」も塞いだ)
⑨**`ideas.md` §2.6 の CSV 写像表の行数 ↔ 同書の自称値 4 パターン ↔ `operations.md` §6.3.1 の告知**
(2026-08-23 追加 — 増分 proto-v4 が CSV 4 列を改称した際、「16 列」が 5 箇所以上に転記されているのに
無検査だとレビューで指摘された (重大 2)。列数・列名は実装リポの CSV 生成テストの期待値になる)。

> **この一覧は検査を増やすたびに必ず更新する** — 2026-08-23 の再レビューが「本一覧が①〜⑦のまま
> 実体は⑨まである」ことを中指摘 (R2-M1) として挙げた。**一覧が stale だと「無検査」の誤判定を生み、
> 同じ検査が再提起される**。数え上げの詳細はスクリプトのコメントが正 (`scripts/check-endpoint-mapping.sh`)。
...
**故障注入で検出力を確認済み** (**種数はここに転記しない — 検査を足すたびに増えるため。DR-9**。
実施済みの注入: 総覧表の本数を 37→36 に改ざん / §2.6 の対応行を 1 行削除 /
...
本文の内訳を「R-1 3 本」→「4 本」に改ざん (算術破綻) /
**検査⑨ の 4 種** (CSV 写像表 1 行削除 / operations・§2.6.1 の自称値改ざん / 同一文言の 2 個目のみ改ざん = 多重ヒット検査で検出。2026-08-23) — いずれも `exit 1` で検出)。
