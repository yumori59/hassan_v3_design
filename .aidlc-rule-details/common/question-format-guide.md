...
- **このリポジトリでは `questions.md` (増分は `questions-<increment>.md`) に固定する** — `.claude/agents/aidlc-planner.md` の「産物の命名規約」が正。
- (原典の例 `{phase-name}-questions.md` / `requirements-questions.md` 等は使わない — 命名揺れがトレーサビリティを壊すため)
...
- **`> 推奨:` 行は必須** (このリポジトリの規約 — `.claude/agents/aidlc-planner.md` Phase 2 が正)。未回答の質問は推奨案を採用して先へ進む運用のため、推奨の無い質問は進行を止める
...
> 推奨: A (PoC のため最小構成で開始し、必要になった時点で SSO を追加)
...
> 推奨: A (モバイル固有の要件が現時点で無いため Web を既定とする)
...
(ユーザーは推奨と異なる選択肢を選んでよい — `> 推奨:` 行は残したまま `[Answer]:` に記入する)
